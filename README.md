...
| `ZULIP_STREAM_OVERRIDES` | *(empty)* | Per-stream `chatmode` overrides, as a JSON object — see [below](#per-stream-chatmode-overrides) |
...
#### Per-stream chatmode overrides

`ZULIP_CHATMODE` applies to every stream at once. When a bot sits in several
streams that want different behaviour — conversational in its own channel, quiet
unless called in a busy shared one — `ZULIP_STREAM_OVERRIDES` overrides it per
stream:
...
- The value is a JSON object mapping stream name to a settings object.
  `chatmode` is the only supported key.
...
and in practice it is inert in all three modes: it is skipped entirely under
`onmessage`, and under `oncall` and `onchar` it can only set `should_process`
to `False` in cases where it is already `False`. `chatmode` is the setting that
actually decides whether the bot answers an unmentioned message, so that is what
this overrides.
