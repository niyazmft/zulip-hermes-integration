    ...
    async def _handle_message(self, message: dict):
        ...
        if msg_type == "stream":
            ...
            if topic and _topic_sessions_enabled():
                # thread_id is the discriminator Hermes scopes session state by,
                # so setting it to the topic gives each topic its own
                # conversation. Without it every topic in the stream shares one.
